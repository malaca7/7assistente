...
try {
  execSync('git commit -m "feat: bind port 8080 and host 0.0.0.0 for Discloud"', { stdio: 'inherit' });
} catch (e) {}
execSync('git push origin source', { stdio: 'inherit' });

console.log('=== [4/5] Atualizando e Publicando Branch Main ===');
const stageDir = path.join(os.tmpdir(), '7assistente_deploy_stage');
...
if (fs.existsSync('index.js')) fs.cpSync('index.js', path.join(stageDir, 'index.js'));
...
if (fs.existsSync(path.join(stageDir, 'index.js'))) fs.cpSync(path.join(stageDir, 'index.js'), 'index.js');
...
execSync('git add -A server/ discloud.config index.html index.mjs index.js package.json .gitignore', { stdio: 'inherit' });
try {
  execSync('git commit -m "deploy: update main with port 8080 and host 0.0.0.0"', { stdio: 'inherit' });
} catch (e) {}
